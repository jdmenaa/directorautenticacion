...
class ProcessResultsEvaluationRiskTest {

    private ProcessResultsEvaluationRiskService service = new ProcessResultsEvaluationRiskService();

    @Test
    void testServiceID() {
        String id = service.getServiceId();
        assertEquals("processResultsEvaluationRisk",id);
    }
    ...
    void testChannelToBackendXML() {
        try {
            ...
            String xmlin = IOUtils.resourceToString("/processResultsEvaluationRisk/requestIn.xml", StandardCharsets.UTF_8);
            String jsonOut = service.channelToBackend(xmlin);

            String jsonEsp = IOUtils.resourceToString("/processResultsEvaluationRisk/requestOut.json", StandardCharsets.UTF_8);
    ...
    void testBackendToChannelXML() {
        try {
            String jsonIn = IOUtils.resourceToString("/processResultsEvaluationRisk/responseIn.json", StandardCharsets.UTF_8);
            String xmlout = service.backendToChannel(jsonIn);

            String xmlesp = IOUtils.resourceToString("/processResultsEvaluationRisk/responseOut.xml", StandardCharsets.UTF_8);
